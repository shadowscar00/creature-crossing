defmodule CreatureCrossing.Nookipedia.WikiScraper do
  ...
  def fetch_villager(name) do
    with {:ok, wikitext} <- fetch_wikitext(name, :villager) do
      ...
      # Reject non-standard cards (Sanrio, Pocket Camp, Welcome, Happy Home, promo)
      amiibo_url =
        if is_binary(raw_amiibo_url) and Regex.match?(~r/Sanrio|Pocket|Welcome|Happy.?Home|Camper|promo/i, raw_amiibo_url) do
          nil
        else
          raw_amiibo_url
        end
